...
def get_version(options: Options) -> Version:
    if options.selected_versions is not None and "ios" in options.selected_versions:
        if type(options.selected_versions["ios"]) == VersionRange:
            filtered = options.selected_versions["ios"].filter(versions)
