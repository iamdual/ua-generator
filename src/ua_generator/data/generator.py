...
from .platforms import linux, android, windows, macos
...
class Generator:
    ...
    def generate_user_agent(self):
        if self.platform == 'windows':
            ...
            if self.browser == 'chrome':
                template = 'Mozilla/5.0 (Linux; Android {android}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.78 Mobile Safari/537.36'
                template = template.replace('{android}', self.platform_version['major'].replace('.0', ''))
                template = template.replace('{webkit}', self.browser_version['webkit'])
                template = template.replace('{chrome}', formats.version(self.browser_version))
                return template
            elif self.browser == 'firefox':
                template = 'Mozilla/5.0 (Android {android}; Mobile; rv:{firefox}) Gecko/68.0 Firefox/{firefox}'
                template = template.replace('{android}', self.platform_version['major'].replace('.0', ''))
                template = template.replace('{firefox}', formats.version(self.browser_version))
                return template
