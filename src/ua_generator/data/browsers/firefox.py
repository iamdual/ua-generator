...
def get_version(options: Options) -> Version:
    if options.selected_versions is not None and "firefox" in options.selected_versions:
        if type(options.selected_versions["firefox"]) == VersionRange:
            filtered = options.selected_versions["firefox"].filter(versions)
